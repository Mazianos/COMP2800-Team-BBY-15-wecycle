...
export default function DonorPost() {
  ...
  useEffect(() => {
    fetch(`/get-own-post/${currentUser.uid}${authorName}`)
      .then((res) => {
        if (res.ok) {
          res.json();
        } else {
  ...
  async function handleUpdate(e) {
    ...
    await fetch("/update-own-post", {
      ...
      .then((res) => res.json())
      .then((result) => {}); // parses JSON response into native JavaScript objects
  }
